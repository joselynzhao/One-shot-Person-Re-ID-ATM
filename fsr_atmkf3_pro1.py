...
def main(args):
    ...
    for step in range(total_step):
        ...
        if iter_mode==2:
            ...
            _, _, select_pre= tagper.generate_new_train_data(selected_idx, pred_y,step,args.experiment) # yml:新增后两个参数
            ...
            KF_score = np.array(
                [KF[i] * (1 - (1 - PE_score[i]) * (1 - AE_score[i])) + (1 - KF[i]) * max(AE_score[i], PE_score[i]) for i
                 in range(len(u_data))])
            KF_label = np.array(
                [KF[i] * AE_y[i] + (1 - KF[i]) * (AE_y[i] if AE_score[i] > PE_score[i] else PE_y[i]) for i in
                 range(len(u_data))])
            # 概率优化
            ...
            new_train_data, unselected_data, kf_select_pre = tagper.generate_new_train_data(selected_idx, KF_label,step,args.experiment)
        ...
        kf_file.write("{} {} {} {:.2%} {:.2%} {:.2%} {:.2%} {:.2%} {:.2%} {:.2%}\n".format(step, nums_to_select,
                                                                                           nums_to_select_tagper,
                                                                                           raw_label_pre,
                                                                                           raw_select_pre,
                                                                                           raw_select_pre_t,
                                                                                           t_label_pre, t_select_pre,
